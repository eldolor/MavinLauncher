...
import com.mavin.launcher.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class LauncherModel extends BroadcastReceiver {
    ...
    // sItemsIdMap maps *all* the ItemInfos (shortcuts, folders, and widgets)
    // created by
    // LauncherModel to their ids
    static final HashMap<Long, ItemInfo> sItemsIdMap = new HashMap<Long, ItemInfo>();

    // sItems is passed to bindItems, which expects a list of all folders and
    // shortcuts created by
    // LauncherModel that are directly on the home screen (however, no widgets
    // or shortcuts
    // within folders).
    static final ArrayList<ItemInfo> sWorkspaceItems = new ArrayList<ItemInfo>();

    // sAppWidgets is all LauncherAppWidgetInfo created by LauncherModel. Passed
    // to bindAppWidget()
    static final ArrayList<LauncherAppWidgetInfo> sAppWidgets =
            new ArrayList<LauncherAppWidgetInfo>();

    // sFolders is all FolderInfos created by LauncherModel. Passed to
    // bindFolders()
    static final HashMap<Long, FolderInfo> sFolders = new HashMap<Long, FolderInfo>();

    // sDbIconCache is the set of ItemInfos that need to have their icons
    ...
    /**
     * Unbinds all the sWorkspaceItems on the main thread, and return a copy of
     * sWorkspaceItems that is save to reference from the main thread.
     */
    ...
    static void updateItemInDatabaseHelper(Context context, final ContentValues values,
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                if (item != modelItem) {
                    // the modelItem needs to match up perfectly with item if
                    // our model is to be
                    // consistent with the database-- for now, just require
                    // modelItem == item
                    String msg = "item: " + ((item != null) ? item.toString() : "null") +
                            "modelItem: " + ((modelItem != null) ? modelItem.toString() : "null") +
                            "Error: ItemInfo passed to " + callingFunction
                ...
                // elsewhere, such
                // as in Workspace.onDrop. Here, we just add/remove them from
                // the list of items
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                if (sItemsIdMap.containsKey(item.id)) {
                    // we should not be adding new items in the db with the same
                    // id
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if (INTENT_GLOBAL_SEARCH_ACTIVITY_CHANGED.equals(action) ||
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean checkItemPlacement(ItemInfo occupied[][][], ItemInfo item) {
            ...
            if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                ...
                // We use the last index to refer to the hotseat and the screen
                // as the rank, so
                // test and update the occupied state accordingly
                if (occupied[Launcher.SCREEN_COUNT][item.screen][0] != null) {
                    Log.e(TAG, "Error loading shortcut into hotseat " + item
                            + " into position (" + item.screen + ":" + item.cellX + ","
                            + item.cellY
        ...
        private void loadWorkspace() {
            ...
            // Load workspace in reverse order to ensure that latest items are
            // loaded first (and
        ...
        private void bindWorkspace() {
            ...
            // Ensure that we don't use the same folders data structure on the
            // main thread
