...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    // mOverScrollX is equal to computeHorizontalScrollOffset() when we're within the normal scroll range. Otherwise
    ...
    // If true, the subclass should directly update computeHorizontalScrollOffset() itself in its computeScroll method
    ...
    public void scrollBy(int x, int y) {
        scrollTo(mUnboundedScrollX + x, computeVerticalScrollOffset() + y);
    }
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (computeHorizontalScrollOffset() != mScroller.getCurrX() || computeVerticalScrollOffset() != mScroller.getCurrY()) {
    ...
    protected void scrollToNewPageWithoutMovingPages(int newCurrentPage) {
        ...
        int delta = newX - computeHorizontalScrollOffset();
...
//    }
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            int leftScreen = 0;
            int rightScreen = 0;
            while (x <= computeHorizontalScrollOffset() && leftScreen < pageCount - 1) {
                leftScreen++;
                x += getScaledMeasuredWidth(getPageAt(leftScreen)) + mPageSpacing;
            }
            rightScreen = leftScreen;
            while (x < computeHorizontalScrollOffset() + screenWidth && rightScreen < pageCount - 1) {
                rightScreen++;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                // Clip to the bounds
                canvas.save();
                canvas.clipRect(computeHorizontalScrollOffset(), computeVerticalScrollOffset(), computeHorizontalScrollOffset() + getWidth(),
                        computeVerticalScrollOffset() + getHeight());
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (xMoved || xPaged || yMoved) {
            if (mUsePagingTouchSlop ? xPaged : xMoved) {
                ...
                mLastMotionX = x;
                mLastMotionXRemainder = 0;
                mTouchX = computeHorizontalScrollOffset();
                mSmoothingTime = System.nanoTime() / NANOTIME_DIV;
                pageBeginMoving();
            }
...
//            mScrollX = mMaxScrollX;
    ...
    int getPageNearestToCenterOfScreen() {
        ...
        int screenCenter = computeHorizontalScrollOffset() + (getMeasuredWidth() / 2);
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; ++i) {
